...
// 注册歌手列表接口路由
function registerSingerList(app) {   //路由内部，向第三方服务发送请求，拿到数据，处理完后响应给前端
  app.get('/api/getSingerList', (req, res) => {
   ...
   //配置数据
    ...
    get(url, {
      ...
      '-': randomKey,
      data
    }).then((response) => {  //第三方服务接受到请求后返回响应
      const data = response.data
      if (data.code === ERR_OK) {  //data.code为0表示数据获取成功
        // 处理歌手列表数据
        const singerList = data.singerList.data.singerlist

        //将singerList处理为singerMap
        // 构造歌手 Map 数据结构
        const singerMap = { //singerMap里面包含若干list，先构造hot数组列表，进而foreach遍历构造剩余key为A-Z的list
          hot: {
            title: HOT_NAME,
            list: map(singerList.slice(0, 10))
          }  //第一个歌手列表list，热hot，取歌手列表前十，通过下面构造map函数，做一层数据映射，构造单个 singer 数据结构
        }

        singerList.forEach((item) => {  //处理数组，取的每位歌手属于的key
          // 把歌手名转成拼音
          ...
          if (key) {
            if (!singerMap[key]) { //判断key键值是否存在，否则新增
              singerMap[key] = {
                title: key,
                list: []
              }
            ...
            // 每个字母下面会有多名歌手
            singerMap[key].list.push(map([item])[0])  //将每个singer元素（map映射后的）添加当前对应key值的list下
          }
        })
        ...
        for (const key in singerMap) {
          ...
          if (item.title.match(/[a-zA-Z]/)) {
            letter.push(item)   //key为字母的singer放入letter中
          } else if (item.title === HOT_NAME) {
            hot.push(item)    //key为hot的singer放入热门hot
          }
        }
        // 按字母顺序排序
        letter.sort((a, b) => {  //将letter排序
        ...
        res.json({
          code: ERR_OK,
          result: {
            singers: hot.concat(letter)  //先放hot，再放letter
          }
        })
      } else {
        res.json(data)
...
function registerSingerDetail(app) {
  app.get('/api/getSingerDetail', (req, res) => {
    ...
    const data = JSON.stringify({
      comm: { ct: 24, cv: 0 },
      singerSongList: {
        method: 'GetSingerSongList',  //传入singerMid，获取指定歌手的详情界面
...
function registerSongsUrl(app) {
  app.get('/api/getSongsUrl', (req, res) => {
    ...
    function process(mid) {
      ...
      const url = `https://u.y.qq.com/cgi-bin/musics.fcg?_=${getRandomVal()}&sign=${sign}`  //将签名拼接到url上，由于发送的是post请求

      // 发送 post 请求  注：自己的前端可以发送get请求，后端代理转发时可以转为post请求
    ...
    return Promise.all(requests).then(() => {
      // 所有请求响应完毕，urlMap 也就构造完毕了，继而响应给前端，完善URL信息
      res.json({
        code: ERR_OK,
        result: {
          map: urlMap
    ...
    get(url, {
      ...
      if (data.code === ERR_OK) {
        res.json({
          code: ERR_OK,
          result: {
            lyric: Base64.decode(data.lyric)  //将原始数据解码
          }
        })
      } else {
        res.json(data)
